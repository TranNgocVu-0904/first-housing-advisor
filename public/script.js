...
// --- CALCULATION FUNCTION FOR WHAT-IF SANDBOX ---
...
function calcBuySandbox(capital, targetPrice, annualInterest) {
    ...
    // // The annualInterest variable is defined by the user.
    const mr = (annualInterest / 100) / 12; 
    const months = 240; // 20-year loan
...
function calcRentSandbox(income, capital, rent, extraSavings, investRate, rentIncrease) {
    ...
    const data = [];
    for (let yr = 1; yr <= 10; yr++) {
        // Profit = (Income - Current rent + Additional savings) * 12 months
        const surplus = (income - currentRent + extraSavings) * 12;
        
        // Profitable capital
        currentCapital = (currentCapital * (1 + investRate / 100)) + (surplus > 0 ? surplus : 0);
        
        //  After one year, the rent will INCREASE by X million VND/month.
...
function generateQuests(data, financials) {
    ...
    const archetype = data.archetype; // Get from API
    let quests = [];
    let nextRank = "";

    // LOGIC FOR CREATING TASKS BASED ON ARCHETYPES
    if (archetype.includes("Accumulating Turtle")) {
        ...
        // Default for other groups (Settled Lord, Nomad Tiger)
        quests = [
    ...
    let completedQuests = 0; // The variable counts the number of tasks completed.
    const totalQuests = quests.length;
    
    // Set the experience bar to 0% at the beginning
    progress.style.width = "0%";

    // Render Quests to the interface
    quests.forEach((q, index) => {
        const div = document.createElement('div');
        
        // Display a hand icon when the mouse hovers over it, indicating that it's clickable.
        ...
        // ADD A CLICK EVENT TO EACH TASK
        div.addEventListener('click', () => {
            ...
            if (!isDone) {
                ...
                div.querySelector('.quest-text').classList.add('line-through', 'opacity-60', 'decoration-[#800000]', 'decoration-2'); // Strike through the text
                div.querySelector('.quest-icon').style.filter = "grayscale(100%) opacity(50%)"; // Blur the icon
                div.querySelector('.done-overlay').classList.replace('scale-x-0', 'scale-x-100'); // Apply background color
                
                completedQuests++;
            } else {
                // // 2. Uncheck the box (if the user changes their mind and clicks again)
            ...
            // 4. Check NEXT RANK UNLOCK
            if (completedQuests === totalQuests) {
                // Completed 3/3, change text, add bounce effect
                ...
                // Not fully completed, return to previous state (Add back the missing red "Next Rank" text)
                nextLevelTitle.innerHTML = `<span class="text-[#722f37] font-bold uppercase">Next Rank:</span> <span class="text-[#6F8A4B] font-bold text-[15px] tracking-normal">${nextRank}</span>`;
                nextLevelTitle.classList.remove('animate-bounce');
            }
        });
...
// Transition function and automatic management of the Back button
function switchScreen(hideEl, showEl, displayType = 'block') {
    ...
    setTimeout(() => {
        ...
        // --- AUTOMATICALLY TURNING THE BACK BUTTON ON/OFF ON THE HEADER ---
        ...
        if (showEl.id === 'part2-container') {
            ...
            // The other screens (Intro, Part 1, Result) will automatically hide the Back button.
            if(backQBtn) backQBtn.classList.add('hidden');
            if(backSubmitBtn) backSubmitBtn.classList.add('hidden');
        }
    }, 400); 
...
// --- FUNCTION TO CREATE A VIBE METER ---
...
// Back Button from Submit Screen
...
document.getElementById('advisorForm').addEventListener('submit', async (e) => {
    ...
    try {
        // Change from fetch('http://localhost:8000/api/analyze') if you want to run it locally:
        ...
        // Save the original data so that the Sandbox can run
        baseFinancials = {
        ...
        // Separate the icon and text from the Python data returned.
        const parts = data.archetype.split(' ');
        const iconFilename = parts[0]; // Example: "eagle.gif"
        const text = parts.slice(1).join(' '); // Exmaple: "Free Eagle"
        ...
        // Wait 1 second for the image to look good, then switch to the Result screen.
        setTimeout(() => {
            switchScreen(loadingContainer, resultContainer);
            
            // Activate the counting effect.
            setTimeout(() => {
...
// --- REAL-TIME SANDBOX PULL-AND-DROP EVENT ---
...
// ==========================================
// WHAT-IF SANDBOX COLORING EFFECT
// ==========================================
...
// ==========================================
// BUTTON HOVER & RESTART EFFECT
// ==========================================

// --- 1. AUTOMATIC HOVER PHOTO SWAP EFFECT FOR ALL BUTTONS ---
document.querySelectorAll('.hover-gif-btn').forEach(btn => {
    const icon = btn.querySelector('img'); // Find the img tag located within that button
    if (icon) {
        const staticSrc = btn.getAttribute('data-static'); // Get the name of the image
        const animSrc = btn.getAttribute('data-anim');     // Get the name of the gif image
...
// --- 2. RESTART BUTTON'S OWN CLICK EVENT ---
const restartBtn = document.getElementById('restart-btn');
if (restartBtn) {
    restartBtn.addEventListener('click', () => {
        // Reset Form & Delete Old Answers
        document.getElementById('advisorForm').reset();
        for (const key in userAnswers) delete userAnswers[key];
        
        // Reset all 5 sandbox sliders back to default settings.
        ...
        // Trigger a virtual "input" event to reset the color range
        const inputEvent = new Event('input', { bubbles: true });
        
        // Change to an array containing 5 Sandbox sliders directly
...
// ======================================================
// SLIDE BAR COLORING EFFECT (MULTIPLE CHOICE QUESTIONS)
// ======================================================
const quizSlider = document.getElementById('current-slider');
const quizActiveColor = '#990011'; // The color of the test
const quizEmptyColor = '#E8E2D2';  // The color of the empty groove
...
function updateQuizSliderFill() {
    ...
    // Calculate percentages (from 1 to 5)
    const percent = ((val - min) / (max - min)) * 100;
    
    // Fill groove with color
...
function showCustomAlert(message) {
    ...
    // Attach the message to the box
    alertText.innerText = message;
    
    alertModal.classList.remove('hidden');
    
    // Activate fade-in and zoom-in effects
    setTimeout(() => {
...
function hideCustomAlert() {
    ...
    // Reverse the effect: fade and shrink
    alertModal.classList.remove('opacity-100');
    alertModal.classList.add('opacity-0');
    alertBox.classList.remove('scale-100');
    alertBox.classList.add('scale-90');
    
    // Wait 300ms for the effect to finish running before completely hiding
    setTimeout(() => {
        alertModal.classList.add('hidden');
    }, 300);
}

/// Capture the event of clicking the "GOT IT" button or clicking outside the black background to close.
